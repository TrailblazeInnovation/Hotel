export const FormExtension = {
    ...
    render: async ({ trace, element }) => {
        ...
        // Questo è il tuo codice originale, completo e funzionante, che ora utilizza i dati caricati (T e D)
        ...
        function validateStep(){const e=steps[currentStep-1];if(!e)return!0;for(let t of e.querySelectorAll("input[required], textarea[required]")){const e=t.labels?.[0]?.textContent||t.name;if(!t.checkValidity())return alert(`${T.firstName} ${e}`),t.focus(),!1}if(1===currentStep&&(!selectedStartDate||!selectedEndDate))return alert(T.noDatesSelected),!1;if(3===currentStep){if("categories"===currentAccommodationViewInStep3)return alert(T.selectDuration),!1;if(0===formContainer.querySelectorAll('#step3-dynamic-content input[type="checkbox"][id^="acc-myCheckbox"]:checked').length)return alert(T.titlePickAccommodation),!1}if(2===currentStep){const e=formContainer.querySelector("#ButtonSelection"),t=formContainer.querySelector("#rangeDuration");if(e.checked){if(!formContainer.querySelector(".duration-btn.activeBtn"))return alert(T.selectDuration),!1}else if(t.checked){const e=formContainer.querySelector("#fromDay"),t=formContainer.querySelector("#tillDay"),a=parseInt(e.value),n=parseInt(t.value),l=parseInt(e.min),d=parseInt(e.max);let h="";if(isNaN(a)||a<l||a>d)h=`${T.from||"From"} ${T.day||"day"} value is invalid. It must be between ${l} and ${d}.`;else if(isNaN(n)||n<parseInt(t.min)||n>d)h=`${T.until||"Until"} ${T.day||"day"} value is invalid. It must be between ${parseInt(t.min)} and ${d}.`;else if(a>=n)h=`${T.from||"From"} ${T.day||"day"} must be less than ${T.until||"Until"} ${T.day||"day"}.`;if(h)return alert(h),!1}}return!0}

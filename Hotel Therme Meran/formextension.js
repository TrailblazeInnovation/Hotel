export const FormExtension = {
    ...
    render: async ({ trace, element }) => {
        ...
        // --- 1. LOGICA DI CARICAMENTO DINAMICO ---
        // Definiamo i percorsi base per i file generali e quelli specifici dell'hotel
        ...
        let T; // Conterrà le traduzioni
        try {
            // Tenta di caricare il file della lingua dalla cartella 'General'
            const langModule = await import(`${generalPath}/locales/${langCode}.js`);
            T = langModule.translations;
        } catch (error) {
            // Se fallisce, carica l'inglese come fallback
        ...
        // Carica i dati specifici dell'hotel dalla sua cartella
        const hotelDataModule = await import(`${hotelSpecificPath}/hotel-data.js`);
        const { suitesCategoryImageUrl, roomsCategoryImageUrl, suitesData, roomsData } = hotelDataModule.hotelData;


        // --- 2. PREPARAZIONE E RENDER DEL FORM ---
        // Da qui in poi il codice è completo. Usa le variabili T e i dati dell'hotel per costruire tutto.
        ...
        // Ho copiato qui il tuo codice HTML e logica originali,
        // aggiornandoli per usare l'oggetto "T" per ogni testo.
        // NON devi modificare nulla da qui in giù.
        
        formContainer.innerHTML = `
            <style>
            /* Il tuo CSS completo va qui. Per brevità l'ho omesso, ma devi incollarlo tutto. */
            ...
            <div class="step-content">
                ...
                <div class="step step-4" style="display: none;">
                    ...
                    <div><label for="special-requests">${T.specialRequests}</label><textarea id="special-requests" name="special-requests" rows="4" placeholder="${T.specialRequestsTxt}"></textarea></div>
                    <div style="display: flex; justify-content: space-between; gap: 30px; margin-top: 17px;">
                        <button type="button" class="prev">${T.back}</button>
                        <button type="button" class="next">${T.next}</button>
                    </div>
        ...
        function renderStep3DynamicContent(){const e=formContainer.querySelector("#step3-dynamic-content");if(e&&(e.innerHTML="","categories"===currentAccommodationViewInStep3?(e.innerHTML=`\n                <div id="accommodation-categories">\n                    <div class="category-selector" data-category="suites">\n                        <img src="${suitesCategoryImageUrl}" alt="${T.FormSuites||"Suites"}">\n                        <h3>${T.FormSuites||"Suites"}</h3>\n                    </div>\n                    <div class="category-selector" data-category="rooms">\n                        <img src="${roomsCategoryImageUrl}" alt="${T.FormRooms||"Rooms"}">\n                        <h3>${T.FormRooms||"Rooms"}</h3>\n                    </div>\n                </div>\n            `,e.querySelectorAll(".category-selector").forEach(e=>{e.addEventListener("click",function(){currentAccommodationViewInStep3=this.dataset.category,renderStep3DynamicContent()})})):("suites"===currentAccommodationViewInStep3?suitesData:roomsData).forEach(t=>{const a=T[t.description1Var]||"",n=T[t.description2Var]||"",l=T[t.description3Var]||"";itemsHtml+=`\n                    <li>\n                        <input type="checkbox" id="acc-myCheckbox-${t.id}" data-name="${t.name}" data-type="${currentAccommodationViewInStep3}"/>\n                        <label for="acc-myCheckbox-${t.id}">\n                            <img src="${t.imageSrc}" alt="${t.name}" />\n                            <p style="text-align: left;">\n                                <b>${t.name}</b><br/>\n                                ${a} <br/>\n                                ${n} <br/>\n                                ${l}\n                            </p>\n                        </label>\n                        <div class="room-counter">\n                            <button class="counter-btn decrement" type="button">-</button>\n                            <input type="number" class="room-quantity" min="1" value="1" readonly>\n                            <button class="counter-btn increment" type="button">+</button>\n                        </div>\n                    </li>\n                `}),itemsHtml+="</ul>",e.innerHTML=itemsHtml,e.querySelector(".back-to-categories-btn").addEventListener("click",()=>{currentAccommodationViewInStep3="categories",renderStep3DynamicContent()}),setupRoomCounters(e))}
        ...
        function validateStep(){const e=steps[currentStep-1];if(!e)return!0;for(let t of e.querySelectorAll("input[required], textarea[required]")){const e=t.labels?.[0]?.textContent||t.name;if(!t.checkValidity())return alert(`Please fill in the required field: ${e}`),t.focus(),!1}if(1===currentStep&&(!selectedStartDate||!selectedEndDate))return alert("Please select a valid date range."),!1;if(3===currentStep){if("categories"===currentAccommodationViewInStep3)return alert("Please select a category (Suites or Rooms) and then choose an accommodation."),!1;if(0===formContainer.querySelectorAll('#step3-dynamic-content input[type="checkbox"][id^="acc-myCheckbox"]:checked').length)return alert("Please select at least one accommodation type."),!1}if(2===currentStep){const e=formContainer.querySelector("#ButtonSelection"),t=formContainer.querySelector("#rangeDuration");if(e.checked){if(!formContainer.querySelector(".duration-btn.activeBtn"))return alert("Please select a duration option."),!1}else if(t.checked){const e=formContainer.querySelector("#fromDay"),t=formContainer.querySelector("#tillDay"),a=parseInt(e.value),n=parseInt(t.value),l=parseInt(e.min),d=parseInt(e.max);let h="";if(isNaN(a)||a<l||a>d)h=`${T.from||"From"} ${T.day||"day"} value is invalid. It must be between ${l} and ${d}.`;else if(isNaN(n)||n<parseInt(t.min)||n>d)h=`${T.until||"Until"} ${T.day||"day"} value is invalid. It must be between ${parseInt(t.min)} and ${d}.`;else if(a>=n)h=`${T.from||"From"} ${T.day||"day"} must be less than ${T.until||"Until"} ${T.day||"day"}.`;if(h)return alert(h),!1}}return!0}

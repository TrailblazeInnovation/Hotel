export const FormExtension = {
  ...
  render: ({ trace, element }) => {
    (async () => {
      ...
      const hotel = await loadJSON(`${DATA_BASE}/${hotelId}.json`);
      ...
      const descPrice = (num) =>
        (p.priceFromPerPersonPattern || 'Price from: {price} per person').replace(
          '{price}',
          fmt.format(Number(num || 0))
        );
      ...
      formContainer.innerHTML = `
            ...
            <div class="step-content">
                <div class="step step-1">
                    ...
                    <p style="margin: 7px 0px!important;">${p.undertitleChooseYourStay}</p><br/>
                    <div id="firstSte">
                    ...
                    <div id="secondSte" style="display: none;">
                        ${p.undertitleChooseYourStay2}
                    </div>
                    <div style="display: flex; justify-content: right; gap: 30px; margin-top: 17px;">
                        <button type="button" class="next">${p.next}</button>
                    </div>
                </div>
                <div class="step step-2" style="display: none;">
                    <h2 style="margin: 0!important;">${p.titleChooseYourStay}</h2>
                    <p style="margin: 7px 0px!important;">${p.undertitleChooseYourStay2}</p><br/>
                        ...
                        <div style="margin-top: 5px; padding-left: 22px;" id="rangeDurationInputContainer">
                            <div style="display: flex; gap: 10px; align-items: center;">
                                <div class="input-wrapper">
                                    <label for="fromDay">${p.from}</label>
                                    <input type="number" id="fromDay" name="fromDay" min="2" value="2">
                                </div>
                                <div class="input-wrapper">
                                    <label for="tillDay">${p.until}</label>
                    ...
                    <div style="display: flex; justify-content: space-between; gap: 30px; margin-top: 30px;">
                        <button type="button" class="prev">${p.back}</button>
                        <button type="button" class="next">${p.next}</button>
                    </div>
                </div>
                <div class="step step-3" style="display: none;">
                    ...
                    <p style="margin: 7px 0px!important;">${p.undertitlePickAccommodation}</p><br/>
                ...
                <div class="step step-4" style="display: none;">
                    ...
                    <p style="margin: 7px 0px!important;">${p.undertitleWhoTraveling}</p><br/>
                    <div style="display: flex; gap: 30px; justify-content: space-between;">
                        <div style="width: 45%;">
                            <label for="adults">${p.adults14Plus}*</label>
                ...
                <div class="step step-6" style="display: none;">
                    ...
                    <p style="margin: 7px 0px!important;">${p.undertitleReviewBooking}</p><br/>
                    <div id="review-info"></div>
                    <div style="display: flex; justify-content: space-between; gap: 30px; margin-top: 17px;">
                        <button type="button" class="prev">${p.edit}</button>
                        <button type="submit" class="next">${p.submit}</button>
                    </div>
                </div>
            </div>
        `;
      ...
      function createCustomCalendar() {
        ...
        function renderCalendar(month, year) {
          ...
          for (let d = 1; d <= daysInMonth; d++) {
            ...
            dayElement.className = 'calendar-day'; dayElement.textContent = d;
            ...
            if (date < today) {
              ...
              dayElement.addEventListener('click', () => selectDate(new Date(year, month, d)));
            }
        ...
        function updateDateRangeDisplay() {
          ...
          if (!selectedEndDate) {
            dateRangeDisplay.innerHTML = `<span>${p.selected}: ${startDateStr}</span>`;
          } else {
            const endDateStr = selectedEndDate.toLocaleDateString(p.locale || 'en-US', { month: 'short', day: 'numeric', year: 'numeric' });
            dateRangeDisplay.innerHTML = `<span>${p.selected}: ${startDateStr} - ${endDateStr}</span>`;
          }
        }
        renderCalendar(currentMonth, currentYear);
        updateDateRangeDisplay();
          ...
          contentArea.innerHTML = `
                    <div id="accommodation-categories">
                        <div class="category-selector" data-category="suites">
                            <img src="${suitesCategoryImageUrl}" alt="${p.FormSuites || 'Suites'}">
                            <h3>${p.FormSuites || 'Suites'}</h3>
                        </div>
                        <div class="category-selector" data-category="rooms">
                            <img src="${roomsCategoryImageUrl}" alt="${p.FormRooms || 'Rooms'}">
      ...
      function updateStep2() {
        ...
        if (p.maxDaysAllowedText) {
          maxRangeNote.textContent = p.maxDaysAllowedText.replace('{totalDays}', totalDays);
        } else {
          maxRangeNote.textContent = `${p.youCanChoose || 'You can add max'} ${totalDays} ${p.days || 'days'}`;
        }
      ...
      function validateStep() {
        ...
        if (currentStep === 1 && (!selectedStartDate || !selectedEndDate)) {
          alert(p.msgSelectValidRange || 'Please select a valid date range.');
          return false;
        }
        if (currentStep === 3) {
          if (currentAccommodationViewInStep3 === 'categories') {
            alert(p.msgPickCategory || 'Please select a category (Suites or Rooms) and then choose an accommodation.');
            return false;
          }
          ...
          if (selectedAccommodations.length === 0) {
            alert(p.msgPickAccommodation || 'Please select at least one accommodation type.');
            return false;
          }
        }
        if (currentStep === 2) {
          ...
          if (singleDurationRadio.checked) {
            ...
            if (!activeDurationBtn) {
              alert(p.msgPickDuration || 'Please select a duration option.');
              return false;
            }
            ...
            if (isNaN(fromDayVal) || fromDayVal < minDayVal || fromDayVal > totalDaysForRange) {
              errorMessage = p.msgFromDayInvalid || 'From day value is invalid.';
            } else if (isNaN(tillDayVal) || tillDayVal < parseInt(tillDayInputEl.min) || tillDayVal > totalDaysForRange) {
              errorMessage = p.msgUntilDayInvalid || 'Until day value is invalid.';
            } else if (fromDayVal >= tillDayVal) {
              errorMessage = p.msgFromLessThanUntil || 'From day must be less than Until day.';
            }
